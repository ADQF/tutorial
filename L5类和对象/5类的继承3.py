# 继承 > 重写 > super()   ()
# 引题1：比如一个计算机系有 电商方向、软件方向，他们的课程70%一样（数学英语C语言）其他30%不一样，子类跟父类方法部分相同。而上节课的例子run方法是子类完全重写覆盖父类方法。
...
stu1.sey_hi()
stu1.work(time=8)
tea1 = Teacher('杨老师', 27, 'male', 10000, 752)
tea1.sey_hi()
...
super(): 调用父类。重用父类属性、方法，减少了代码量。专为继承设计。子类调用完父类相同之处后，个性化的东西可以另外写。
