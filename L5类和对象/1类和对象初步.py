...
# 2.  函数 学生管理作业，先获取用户输入，(假设用户要修改学生)先展示学生列表，在接收用户输入新学员的信息，在代码修改数据，最后提示修改成功。
...
类和实例instance：“人类”和“小明”。类是许多个体的集合，一个抽象名词、统称。‘小明’或者“你自己”或“特普朗”这些都是人类这个集合中一个个具体的人、个体。那么我们把类中具体的个体叫“对象”、“实例”。再举几个例子：水果类包含苹果、梨。电子产品类包括电脑、手机。手机类包括苹果，华为。
属性attribute，成员方法：一类事物的特性叫做属性，例如Student类中的name、score、sex。类中个体、成员具备的功能、方法叫做成员方法。

语法：类关键字class 类名
驼峰命名：（约定俗成）每一个单词挨着写并且首字母大写。例如：ElectronicProducts   (电子产品)   Java中常见。
...
属性、成员方法：访问属性 对象.属性名；调和成员方法实例.方法；写属性 对象.属性名=新值。成员方法把实例中的属性读出来并书写功能。
