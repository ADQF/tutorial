...
# （不推荐）外层访问内层声明的变量。
# 外层先声明，内层在访问：if  foe  语句块都可以调用外层声明过的变量。同一级的代码可以访问同一级之前声明过的变量。
...
# 局部变量作用域：声明这个局部变量得函数内。不同函数之间不能互相调用各自的局部变量。局部变量让不同函数之间不同函数的变量相互隔离。
...
# f1()
# 全局套着函数1， 函数1套着函数2。调用变量时会在现在当前函数中查找，如果找不到的话会去上一级函数的作用域中查找。 当前层 < 复层 < 最外层全局.
