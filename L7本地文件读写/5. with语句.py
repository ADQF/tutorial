...
# with
with open('chinese_utf-8.txt', 'r', encoding='utf8')as f:
    # f.__enter__()   # 文件打开的预处理
    content = f.read()
    print(content)
    # f.__exit__()    # 对象结束后的操作，例如f.close()


"""
with: with语句要求后面跟的对象实现  进入方法__enter__()  退出方法__exit__()
as: 把…当做…。跟等号相似。
场景：一项任务他有固定的预处理和退出处理，把这些任务的实现代码封装到__enter__() __exit__()中，这样在with语句块中只用写主要的业务逻辑，代码更加清晰。
"""
